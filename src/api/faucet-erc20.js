...
module.exports = {
  async request(req, res) {
    ...
    try {
      ...
      logger(
        `SENT ERC20: ${config.ERC20_PAYOUT} ${config.ERC20_NAME} to ${address}`
      );
      logger(
        `REMAINING ERC20: ${remainingBalance} ${
          config.ERC20_NAME
        } left in faucet`
      );
  ...
  async getStatus(req, res) {
    ...
    return res.json({
      balance,
      address: erc20wallet.getAddress(),
      currency: config.ERC20_NAME.toUpperCase()
    });
  }
};
